...
interface HomeServicesPortalPageProps {
  params: {
    token: string;
  };
}
...
export async function generateMetadata({ params }: HomeServicesPortalPageProps) {
  ...
  return {
    title: `Home Services Portal - ${customerName}',
    description: 'Welcome to your home services portal, ${customerName}',
    robots: 'noindex, nofollow', // Prevent search engine indexing of customer portals
  };
}

export default async function HomeServicesPortalPage({ params }: HomeServicesPortalPageProps) {
  const portalAccess = await validatePortalAccess(params.token);
  ...
  await supabase
    ...
    .insert({
      ...
      activity_description: 'Customer accessed home services portal',
      metadata: {
