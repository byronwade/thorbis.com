...
interface RetailPortalPageProps {
  params: {
    token: string;
  };
}
...
export async function generateMetadata({ params }: RetailPortalPageProps) {
  ...
  return {
    title: `Retail Portal - ${customerName}',
    description: 'Welcome to your retail customer portal, ${customerName}',
    robots: 'noindex, nofollow', // Prevent search engine indexing of customer portals
  };
}

export default async function RetailPortalPage({ params }: RetailPortalPageProps) {
  const portalAccess = await validatePortalAccess(params.token);
  ...
  await supabase
    ...
    .insert({
      ...
      activity_description: 'Customer accessed retail portal',
      metadata: {
