...
import { AdvancedCSRToolbar } from "@components/voip/advanced-csr-toolbar"
import { Button } from "@components/ui/button"
import { Users, ClipboardList, History, Copy, X, PhoneOff, ChevronUp } from "lucide-react"
import { TB_BTN } from "@components/voip/toolbar-classes"
import { useToast } from "@hooks/use-toast"
import QuickGlanceProfile from "@components/voip/quick-glance-profile"
import { createRealtimeClient } from "@lib/realtime"
import IntakeForm from "@components/voip/widgets/intake-form"
import ServiceCatalog from "@components/voip/widgets/service-catalog"
import QuoteBuilder from "@components/voip/widgets/quote-builder"
import DispatchHelper from "@components/voip/widgets/dispatch-helper"
import AccessProperty from "@components/voip/widgets/access-property"
import ComplianceConsent from "@components/voip/widgets/compliance-consent"
import VoipIncomingOverlay from "@components/voip/voip-incoming-overlay"
import { SmartSuggestions } from "@components/voip/modern-csr-enhancements"
import CustomerHistory from "@components/voip/widgets/customer-history"
import PaymentProcessing from "@components/voip/widgets/payment-processing"
import KnowledgeBase from "@components/voip/widgets/knowledge-base"
import QuickActions from "@components/voip/widgets/quick-actions"
import EscalationTracker from "@components/voip/widgets/escalation-tracker"
import AppointmentScheduler from "@components/voip/widgets/appointment-scheduler"
import CommunicationLog from "@components/voip/widgets/communication-log"
import SentimentAnalysis from "@components/voip/widgets/sentiment-analysis"
import SlaMonitor from "@components/voip/widgets/sla-monitor"
import TeamChat from "@components/voip/widgets/team-chat"
import WeatherForecast from "@components/voip/widgets/weather-forecast"
import PredictiveIntelligence from "@components/voip/predictive-intelligence"
import IoTIntegration from "@components/voip/iot-integration"
import ARAssistance from "@components/voip/ar-assistance"
...
export default function CallPage({ params }) {
  ...
  const [contact, setContact] = useState(() => {
    ...
    try {
      ...
      return raw ? JSON.parse(raw) : { name: caller.name, phone: caller.number, email: "" }
    } catch {
  ...
  const [me, setMe] = useState(() => {
    ...
    const u = { id: "self", name: "You", role: "CSR" }
    try {
      window.localStorage.setItem("voip:user", JSON.stringify(u))
    } catch {}
    return u
  })
  const updateIdentity = useCallback((u) => {
    setMe(u)
    try {
      window.localStorage.setItem("voip:user", JSON.stringify(u))
    } catch {}
  ...
  const suggestionClearRef = useRef(null)

  // Review + plan details for header
  const [reviewStars, setReviewStars] = useState(null)
  const [reviewDate, setReviewDate] = useState(null)
  const [maintenancePlan, setMaintenancePlan] = useState("Gold Plan")
  ...
  const setTyping = useCallback(
    (typing) => {
      const client = rtRef.current
      const user = { ...me, id: client.selfId }
      client.emit({ type: "typing", payload: { user, typing } })
    },
    [me],
  )

  const broadcastProfile = useCallback((p) => {
...
FieldOps`
  ...
  return (
    <TooltipProvider>
      <main className="min-h-[100dvh] bg-gradient-to-br from-neutral-950 via-neutral-900 to-neutral-950 text-neutral-100 fixed inset-0 z-50 overflow-hidden">
        <AdvancedCSRToolbar
          callId={id}
          caller={caller}
        ...
        <section className="flex-1 flex flex-col lg:flex-row h-[calc(100vh-200px)]">
