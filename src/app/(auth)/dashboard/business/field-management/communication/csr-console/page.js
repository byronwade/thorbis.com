...
import { VoipQuickActions } from '@components/voip/VoipTrigger'
import { useVoipGlobal } from '@context/voip-global-context'

export default function CSRConsolePage() {
  const { isCallActive, callData: currentCustomer } = useVoipGlobal()
  ...
  const openVoipInterface = (customerId) => {
    const voipUrl = `/dashboard/business/voip-call/${encodeURIComponent(customerId)}`
    window.open(voipUrl, 'voip-interface', 'width=1400,height=900,scrollbars=yes,resizable=yes')
  }

  const openCallInterface = (customerId) => {
    const callUrl = `/dashboard/business/voip-call/${encodeURIComponent(customerId)}`
