...
export const useLocalHubDashboard = () => {
	...
	// Ensure auth store initializes so user state becomes available
	useEffect(() => {
	...
	// Initialize dashboard on auth resolution; stop loading if unauthenticated
	useEffect(() => {
		if (user) {
			initializeDashboard();
		} else if (loading === false) {
			// Prevent perpetual loading when no authenticated user
			setIsLoading(false);
		}
	}, [user, loading, initializeDashboard]);
