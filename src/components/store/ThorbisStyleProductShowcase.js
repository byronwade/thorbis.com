...
const ThorbisStyleProductShowcase = ({ product }) => {
        ...
        <div className="max-w-6xl mx-auto px-4 lg:px-6 w-full relative z-10">
            ...
            {bentoGridItems.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`absolute -inset-1 bg-gradient-to-r ${item.color} opacity-0 group-hover:opacity-30 blur-xl transition-opacity duration-500`} />
                  ...
                  <div className="absolute inset-0 overflow-hidden">
                    {Array.from({ length: 3 }).map((_, i) => (
                      <motion.div
                        key={i}
                        className={`absolute w-1 h-1 bg-gradient-to-r ${item.color} rounded-full opacity-60`}
                        style={{
                          left: `${20 + i * 30}%`,
                          top: `${20 + i * 20}%`,
                        }}
                  ...
                  <div className="absolute inset-0 opacity-5">
                    <div className="grid grid-cols-3 gap-2 h-full">
                      {Array.from({ length: 9 }).map((_, i) => (
                        <motion.div
                          key={i}
                          className={`bg-gradient-to-r ${item.color} rounded-sm`}
                          animate={{
                            scale: [1, 1.1, 1],
                            opacity: [0.3, 0.6, 0.3],
                          }}
                  ...
                  <div className="relative z-10 h-full flex flex-col">
                    ...
                    <motion.div 
                      className={`w-12 h-12 lg:w-16 lg:h-16 bg-gradient-to-r ${item.color} rounded-2xl flex items-center justify-center mb-4 lg:mb-6 shadow-2xl relative`}
                      whileHover={{ 
                        scale: 1.1,
                        rotateY: 15,
                        transition: { duration: 0.3 }
                    ...
                    >
                      {/* Icon Glow */}
                      <div className={`absolute inset-0 bg-gradient-to-r ${item.color} rounded-2xl blur-md opacity-50`} />
                      <IconComponent className="w-6 h-6 lg:w-8 lg:h-8 text-white relative z-10" />
                      
                      {/* Floating Elements around Icon */}
                      <motion.div
                        className={`absolute -top-1 -right-1 w-3 h-3 bg-gradient-to-r ${item.color} rounded-full opacity-80`}
                        animate={{
                          scale: [1, 1.3, 1],
                          opacity: [0.8, 1, 0.8],
                        }}
                    ...
                    <div className="flex-1">
                      ...
                      {item.subtitle && (
                        <motion.p 
                          className="text-sm lg:text-base text-gray-300 mb-3"
                    ...
                    >
                      <motion.div 
                        className={`w-8 h-8 bg-gradient-to-r ${item.color} rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 shadow-lg`}
                        whileHover={{ 
                          scale: 1.1,
                          rotate: 5,
                        }}
                    ...
                    <motion.div
                      ...
                      style={{
                        background: `linear-gradient(45deg, transparent, transparent, ${item.color.split(' ')[1]}, transparent, transparent)`,
          ...
          >
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white px-8 py-4 rounded-2xl font-bold text-lg shadow-2xl hover:shadow-blue-500/25 transition-all duration-300"
            >
  ...
  const renderFeatureSection = (feature, index) => {
    ...
    // Scroll-driven animations with performance optimization
    ...
    const getAnimationValues = (type) => {
      switch (type) {
        case 'slide':
          return {
            contentX: useTransform(scrollYProgress, [0, 0.3, 0.7, 1], [-100, 0, 0, 100]),
            visualX: useTransform(scrollYProgress, [0, 0.3, 0.7, 1], [100, 0, 0, -100]),
            opacity: useTransform(scrollYProgress, [0, 0.2, 0.8, 1], [0, 1, 1, 0])
          };
        case 'fade':
          return {
            contentX: useTransform(scrollYProgress, [0, 0.5, 1], [0, 0, 0]),
            visualX: useTransform(scrollYProgress, [0, 0.5, 1], [0, 0, 0]),
        ...
        case 'rotate':
          return {
            ...
            visualX: useTransform(scrollYProgress, [0, 0.5, 1], [0, 0, 0]),
            opacity: useTransform(scrollYProgress, [0, 0.2, 0.8, 1], [0, 1, 1, 0]),
            rotate: useTransform(scrollYProgress, [0, 0.5, 1], [-5, 0, 5])
          };
        case 'float':
          return {
            contentX: useTransform(scrollYProgress, [0, 0.5, 1], [0, 0, 0]),
        ...
        case 'pulse':
          return {
            ...
            visualX: useTransform(scrollYProgress, [0, 0.5, 1], [0, 0, 0]),
            opacity: useTransform(scrollYProgress, [0, 0.2, 0.8, 1], [0, 1, 1, 0]),
            scale: useTransform(scrollYProgress, [0, 0.25, 0.5, 0.75, 1], [1, 1.05, 1, 1.05, 1])
          };
        default:
          return {
            contentX: useTransform(scrollYProgress, [0, 0.3, 0.7, 1], [-50, 0, 0, 50]),
            visualX: useTransform(scrollYProgress, [0, 0.3, 0.7, 1], [50, 0, 0, -50]),
            opacity: useTransform(scrollYProgress, [0, 0.2, 0.8, 1], [0, 1, 1, 0])
          };
      }
    };

    const animValues = getAnimationValues(feature.animationType);
    
    // Spring animations for smooth interpolation
    const smoothContentX = useSpring(animValues.contentX, { 
      stiffness: 300, 
      damping: 30,
      mass: 0.5
    });
    const smoothVisualX = useSpring(animValues.visualX, { 
      stiffness: 300, 
      damping: 30,
      mass: 0.5
    });
    const smoothOpacity = useSpring(animValues.opacity, { 
      stiffness: 400, 
      damping: 40
    });
    ...
    const renderSplitLayout = () => (
          ...
          style={{
            ...
            >
                  <IconComponent className={`w-4 h-4 bg-gradient-to-r ${feature.color} bg-clip-text text-transparent`} />
                  <span className="text-xs font-semibold text-gray-300">{feature.subtitle}</span>
            </motion.div>
    ...
    const renderStackedLayout = () => (
      <div className="text-center space-y-8">
        ...
        <motion.div
          ...
          className="space-y-4"
        >
          <div className={`w-20 h-20 bg-gradient-to-r ${feature.color} rounded-3xl flex items-center justify-center mx-auto shadow-2xl`}>
        ...
        >
          <div className={`${feature.bgColor} ${feature.borderColor} border rounded-2xl p-6`}>
            ...
            <div className="text-sm text-gray-400 mt-2">{feature.detail}</div>
          </div>
          <div className={`${feature.bgColor} ${feature.borderColor} border rounded-2xl p-6`}>
            <div className="text-3xl font-bold text-white">{feature.subtitle}</div>
            <div className="text-sm text-gray-400 mt-2">Hardware Security</div>
          </div>
          <div className={`${feature.bgColor} ${feature.borderColor} border rounded-2xl p-6`}>
            <div className="text-3xl font-bold text-white">PCI</div>
            <div className="text-sm text-gray-400 mt-2">Certified</div>
          </div>
        </motion.div>
      </div>
    );
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0 opacity-10">
          <div className="grid grid-cols-8 gap-4 h-full">
            {Array.from({ length: 64 }).map((_, i) => (
              <div key={i} className={`bg-gradient-to-r ${feature.color} rounded-lg`} />
            ))}
          </div>
        </div>
        ...
        <div className="relative z-10 text-center space-y-8">
          <motion.div
            ...
            className="space-y-6"
          >
            <div className={`w-24 h-24 bg-gradient-to-r ${feature.color} rounded-full flex items-center justify-center mx-auto shadow-2xl`}>
      ...
      <div className="space-y-8">
        ...
        >
          <div className={`${feature.bgColor} ${feature.borderColor} border rounded-2xl p-6 text-center`}>
            <Wifi className={`w-8 h-8 mx-auto mb-4 bg-gradient-to-r ${feature.color} bg-clip-text text-transparent`} />
            <h3 className="text-xl font-bold text-white mb-2">Mobile Terminals</h3>
            <p className="text-sm text-gray-400">Wireless connectivity</p>
          </div>
          <div className={`${feature.bgColor} ${feature.borderColor} border rounded-2xl p-6 text-center`}>
            <Monitor className={`w-8 h-8 mx-auto mb-4 bg-gradient-to-r ${feature.color} bg-clip-text text-transparent`} />
            <h3 className="text-xl font-bold text-white mb-2">Kitchen Displays</h3>
            <p className="text-sm text-gray-400">Real-time updates</p>
          </div>
          <div className={`${feature.bgColor} ${feature.borderColor} border rounded-2xl p-6 text-center`}>
            <Globe className={`w-8 h-8 mx-auto mb-4 bg-gradient-to-r ${feature.color} bg-clip-text text-transparent`} />
      ...
      <div className="space-y-8">
        ...
        <motion.div
          style={{
            ...
            >
          {/* Timeline Line */}
          <div className="absolute left-1/2 transform -translate-x-1/2 w-1 h-full bg-gradient-to-b from-gray-800 to-gray-600" />
          
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              <div className={`${feature.bgColor} ${feature.borderColor} border rounded-2xl p-6 w-5/12`}>
                <div className="flex items-center gap-3 mb-2">
                  <Battery className={`w-5 h-5 bg-gradient-to-r ${feature.color} bg-clip-text text-transparent`} />
                  <span className="text-sm font-semibold text-gray-300">UPS Battery</span>
                </div>
                <p className="text-sm text-gray-400">Built-in backup power</p>
              </div>
              <div className={`w-4 h-4 bg-gradient-to-r ${feature.color} rounded-full`} />
              <div className={`${feature.bgColor} ${feature.borderColor} border rounded-2xl p-6 w-5/12`}>
                <div className="flex items-center gap-3 mb-2">
                  <Wifi className={`w-5 h-5 bg-gradient-to-r ${feature.color} bg-clip-text text-transparent`} />
                  <span className="text-sm font-semibold text-gray-300">Offline Mode</span>
                </div>
                <p className="text-sm text-gray-400">Never miss a sale</p>
              </div>
            </div>
        ...
        case 'gradient':
          return (
              <div className={`relative ${feature.bgColor} ${feature.borderColor} border rounded-2xl p-6 lg:p-8 shadow-2xl overflow-hidden`}>
                ...
                <div className={`absolute -inset-1 bg-gradient-to-r ${feature.color} opacity-20 blur-lg`} />
                <div className="relative z-10">
                  <div className="aspect-square bg-black/20 backdrop-blur-sm rounded-xl flex items-center justify-center border border-white/10">
                    <div className="text-center space-y-4">
                      <div className={`w-16 h-16 bg-gradient-to-r ${feature.color} rounded-2xl flex items-center justify-center mx-auto shadow-2xl`}>
              ...
              <div className={`${feature.bgColor} ${feature.borderColor} border rounded-2xl p-8`}>
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    <div className={`w-3 h-3 bg-gradient-to-r ${feature.color} rounded-full`} />
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <IconComponent className={`w-6 h-6 bg-gradient-to-r ${feature.color} bg-clip-text text-transparent`} />
                  ...
                  <div className="bg-black/20 rounded-lg p-4">
                    ...
                    <div className="text-sm text-gray-400">{feature.detail}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          return (
            <div className="relative">
              <div className={`${feature.bgColor} ${feature.borderColor} border rounded-2xl p-8`}>
                <div className="flex items-center justify-center mb-6">
                  <div className="relative">
                    <div className={`w-20 h-20 bg-gradient-to-r ${feature.color} rounded-full flex items-center justify-center`}>
                      <IconComponent className="w-10 h-10 text-white" />
                    </div>
                ...
                <div className="space-y-4">
                  <div className="bg-black/20 rounded-lg p-4">
                    ...
                    <div className="w-full bg-gray-700 rounded-full h-2">
                      <div className={`bg-gradient-to-r ${feature.color} h-2 rounded-full`} style={{ width: '100%' }} />
                    </div>
                  </div>
                  <div className="text-center">
  ...
  const renderTestimonialsSection = () => {
    ...
    const smoothOpacity = useSpring(useTransform(scrollYProgress, [0, 0.5, 1], [0, 1, 1]), {
      stiffness: 300,
      damping: 30
    });
        ...
        <div className="max-w-6xl mx-auto px-4 lg:px-6 w-full relative z-10">
          ...
          >
            <div className="bg-black/20 rounded-2xl p-8 border border-white/10">
              <p className="text-lg text-gray-300 italic">"Thorbis POS Lite has revolutionized our checkout process. The lightning-fast speed and robust security features have made it a game-changer for our retail store."</p>
              ...
              <p className="mt-4 text-sm text-gray-400">Sarah Johnson, Owner of TechMart</p>
            </div>
            <div className="bg-black/20 rounded-2xl p-8 border border-white/10">
              <p className="text-lg text-gray-300 italic">"The industrial-grade durability and offline-first architecture have made Thorbis POS Lite a reliable partner for our food truck operations. No more power outages affecting sales!"</p>
              <div className="mt-6 flex items-center gap-2">
                <Star className="w-5 h-5 text-muted-foreground" />
                <Star className="w-5 h-5 text-muted-foreground" />
                <Star className="w-5 h-5 text-muted-foreground" />
              ...
              <p className="mt-4 text-sm text-gray-400">Mike Chen, Manager of Foodie Junction</p>
            </div>
            <div className="bg-black/20 rounded-2xl p-8 border border-white/10">
              <p className="text-lg text-gray-300 italic">"The seamless ecosystem of mobile terminals, kitchen displays, and kiosks has transformed our restaurant operations. Thorbis POS Lite truly is a complete solution."</p>
              <div className="mt-6 flex items-center gap-2">
                <Star className="w-5 h-5 text-muted-foreground" />
                <Star className="w-5 h-5 text-muted-foreground" />
                <Star className="w-5 h-5 text-muted-foreground" />
              ...
              <p className="mt-4 text-sm text-gray-400">Lisa Rodriguez, Manager of Local Market</p>
            </div>
            </motion.div>
          </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4 lg:px-6 w-full relative z-10">
          ...
          >
            <div className="bg-black/20 rounded-2xl p-8 border border-white/10">
              ...
              <p className="text-gray-300">Thorbis POS Lite is powered by a custom ARM processor, ensuring unparalleled speed and reliability. The TPM 2.0 security module provides military-grade encryption, safeguarding your sensitive data.</p>
            </div>
            <div className="bg-black/20 rounded-2xl p-8 border border-white/10">
              <h3 className="text-2xl font-bold text-white mb-4">Powerful Software</h3>
              <p className="text-gray-300">Our proprietary software is designed to be lightweight and efficient, yet feature-rich. It seamlessly integrates with your existing infrastructure and can be deployed in minutes.</p>
            </div>
            <div className="bg-black/20 rounded-2xl p-8 border border-white/10">
              <h3 className="text-2xl font-bold text-white mb-4">Seamless Connectivity</h3>
              <p className="text-gray-300">With built-in Wi-Fi, Bluetooth, and cellular connectivity, Thorbis POS Lite can operate in virtually any environment. It's also compatible with a wide range of peripherals and payment gateways.</p>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4 lg:px-6 w-full relative z-10">
          ...
          >
            <div className="bg-black/20 rounded-2xl p-8 border border-white/10">
              ...
              <p className="text-gray-300">Absolutely. Thorbis POS Lite is designed to be versatile and adaptable to any retail, restaurant, or service-based business. Whether you're a small shop or a large enterprise, we have a solution for you.</p>
            </div>
            <div className="bg-black/20 rounded-2xl p-8 border border-white/10">
              <h3 className="text-2xl font-bold text-white mb-4">How long does it take to set up Thorbis POS Lite?</h3>
              <p className="text-gray-300">Our goal is to make it as quick and painless as possible. With our intuitive software and user-friendly interface, you can be up and running in just a few minutes. We also offer comprehensive training and support.</p>
            </div>
            <div className="bg-black/20 rounded-2xl p-8 border border-white/10">
              <h3 className="text-2xl font-bold text-white mb-4">What are the system requirements for Thorbis POS Lite?</h3>
              <p className="text-gray-300">Thorbis POS Lite is a lightweight application that runs on most modern devices. You'll need a computer with a minimum of 2GB RAM, a stable internet connection, and a modern web browser. For optimal performance, we recommend a dedicated device.</p>
            </div>
            <div className="bg-black/20 rounded-2xl p-8 border border-white/10">
              <h3 className="text-2xl font-bold text-white mb-4">Is my data secure with Thorbis POS Lite?</h3>
              <p className="text-gray-300">Absolutely. Thorbis POS Lite implements the highest security standards, including military-grade encryption (TPM 2.0) and offline-first architecture. Your data is encrypted in transit and at rest, and you have full control over your security settings.</p>
          </div>
          </motion.div>
        </div>
      </section>
