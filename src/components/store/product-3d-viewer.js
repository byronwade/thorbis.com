...
export default function Product3DViewer({ 
  ...
  return (
    <div className={`relative bg-white dark:bg-neutral-950 rounded-3xl border border-border dark:border-neutral-800 overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 ${className}`}>
      ...
      <div className="absolute top-0 left-0 right-0 z-10 p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block text-sm text-muted-foreground dark:text-muted-foreground font-medium">
              {productName}
            </div>
          </div>
          ...
          {showControls && (
            <div className="flex items-center gap-2">
              <Button
                ...
                size="sm"
                onClick={resetView}
                className="h-10 w-10 p-0 bg-white/90 hover:bg-white dark:bg-neutral-800/90 dark:hover:bg-neutral-800 shadow-md border border-border dark:border-neutral-700 transition-all duration-200"
                title="Reset view"
              >
      ...
      {isLoading && (
        <div className="absolute inset-0 flex items-center justify-center bg-white/95 dark:bg-neutral-950/95 backdrop-blur-sm">
          <div className="text-center">
            ...
            <h3 className="text-lg font-semibold text-foreground dark:text-white mb-2">Loading 3D Experience</h3>
            <p className="text-sm text-muted-foreground dark:text-muted-foreground">Preparing interactive product view...</p>
          </div>
        </div>
      )}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6">
        <div className="bg-white/95 dark:bg-neutral-900/95 backdrop-blur-sm rounded-2xl border border-border/50 dark:border-neutral-700/50 p-4 shadow-lg">
          <div className="flex items-center justify-center gap-6 text-sm text-muted-foreground dark:text-muted-foreground">
            <div className="flex items-center gap-2">
              <div className="w-8 h-5 border-2 border-border dark:border-border rounded-sm flex items-center justify-center">
            ...
            <div className="flex items-center gap-2">
              <div className="w-6 h-4 border-2 border-border dark:border-border rounded-sm relative">
                <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-1 h-1 bg-muted rounded-full"></div>
              </div>
      ...
      {isFullscreen && (
        <div className="absolute top-6 right-6 z-20">
          <Button
            ...
            size="sm"
            onClick={toggleFullscreen}
            className="h-12 w-12 p-0 bg-white/95 hover:bg-white dark:bg-neutral-800/95 dark:hover:bg-neutral-800 shadow-lg border border-border dark:border-neutral-700 rounded-xl transition-all duration-200"
          >
            <X className="h-5 w-5" />
          </Button>
        </div>
      ...
      <div className="absolute top-20 right-6 z-10">
        <div className="bg-white/90 dark:bg-neutral-900/90 backdrop-blur-sm rounded-xl p-3 shadow-lg border border-border/50 dark:border-neutral-700/50 max-w-xs">
          <div className="text-xs font-medium text-foreground dark:text-white mb-1">Premium Build Quality</div>
          <div className="text-xs text-muted-foreground dark:text-muted-foreground">360° interactive view • Professional materials</div>
        </div>
      </div>
    </div>
  );
