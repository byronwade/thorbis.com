...
const generateMainNavigation = (fromIndustry?: string) => {
  ...
  const navigation = [
    { name: "Dashboard", href: "/dashboards/money", icon: DollarSign, description: "Financial management overview" },
  ...
  if (!fromIndustry || fromIndustry !== 'hs') {
    navigation.splice(1, 0, { name: "Home Services", href: '/dashboards/hs${fromParam}', icon: Home, description: "Home Services dashboard" })
  }
  
  return navigation
}

// Money-specific navigation
const moneyNavigation = [
  {
    title: "Revenue",
    items: [
      { name: "Invoices", href: "/dashboards/money/invoices", icon: Receipt, description: "Billing and collections" },
      { name: "Estimates", href: "/dashboards/money/estimates", icon: Calculator, description: "Quotes and proposals" },
      { name: "Payments", href: "/dashboards/money/payments", icon: CreditCard, description: "Payment processing" },
      { name: "Recurring", href: "/dashboards/money/recurring", icon: RefreshCw, description: "Subscription billing" }
    ]
  },
  {
    title: "Expenses",
    items: [
      { name: "Bills", href: "/dashboards/money/bills", icon: FileText, description: "Vendor bills and expenses" },
      { name: "Purchase Orders", href: "/dashboards/money/purchase-orders", icon: ShoppingCart, description: "Ordering and procurement" },
      { name: "Inventory Costs", href: "/dashboards/money/inventory-costs", icon: Package, description: "Material and supply costs" }
    ]
  },
  {
    title: "Reports",
    items: [
      { name: "P&L Statement", href: "/dashboards/money/profit-loss", icon: TrendingUp, description: "Profit and loss reports" },
      { name: "Cash Flow", href: "/dashboards/money/cash-flow", icon: DollarSign, description: "Cash flow analysis" },
      { name: "Tax Reports", href: "/dashboards/money/tax-reports", icon: Calculator, description: "Tax preparation reports" }
    ]
  }
]

export function MoneySidebar({ user: _user, parentIndustry }: MoneySidebarProps) {
  ...
  return (
    <Sidebar variant="inset" className="sidebar-content">
            ...
            <SidebarMenuButton size="lg" asChild>
              <Link href="/dashboards" className="relative">
                ...
                <div className="grid flex-1 text-left text-sm leading-tight">
                  <span className="truncate font-semibold">Money Dashboard</span>
      ...
      <SidebarContent>
        ...
        {/* Money-Specific Navigation */}
        {moneyNavigation.map((section, sectionIndex) => {
