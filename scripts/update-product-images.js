...
function updateProductImages() {
  ...
  // Import the products to get the actual data
  ...
  for (const product of allProducts) {
    ...
    const oldImagePattern = new RegExp(`image:\\s*"[^"]*",\\s*//\\s*${product.id}`, 'g');
    const newImageLine = `image: "${newImagePath}", // ${product.id}`;
    
    // Replace the image line
    productsContent = productsContent.replace(
      /image:\s*"[^"]*",/g,
      (match, index) => {
        // Find the product this image belongs to by looking at the context
        const beforeMatch = productsContent.substring(Math.max(0, index - 200), index);
        const afterMatch = productsContent.substring(index, index + 200);
        
        // Check if this is the right product by looking for the product ID
        if (beforeMatch.includes(product.id) || afterMatch.includes(product.id)) {
